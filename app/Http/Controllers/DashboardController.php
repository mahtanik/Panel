...
class dashboardController extends Controller
{
    public function counter()
    {
        ...
        $autocharges = DB:: table('tb_transactions')->select('action')->where('action' , 'AUTO_CHARGE')->count();

        $unsubs = DB::table('tb_transactions')->select('action')->where('action' , 'UNSUBSCRIPTION')->count();

        $subs = DB::table('tb_transactions')->select('action')->where('action', 'SUBSCRIPTION')->count();

        $total =DB::table('tb_users')->select('status')->where('status' , 'sub')->count();

        $totalcharge = DB::table('tb_transactions')->select('action')->where('action' , 'AUTO_CHARGE')->orWhere('action' , 'SUBSCRIPTION')->count();
